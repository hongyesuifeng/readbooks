...
graph TB
    ...
    B --> |每层内按功能拆模块| C[依赖倒置]
    C --> |高层不依赖低层实现| D[接口隔离]
    D --> |通过接口解耦具体类| E0[解耦与可替换]
    
    subgraph layered_example["分层架构示例"]
...
```mermaid
graph TB
    subgraph view_layer["表现层 - 负责显示"]
        V1[PlayerView] 
        V2[EnemyView]
        V3[UIView]
    end
    
    subgraph logic_layer["逻辑层 - 负责业务"]
        C1[PlayerController]
        C2[EnemyController]
        C3[GameController]
    end
    
    subgraph data_layer["数据层 - 负责存取"]
        M1[PlayerModel]
        M2[GameDataModel]
        M3[ConfigData]
    end
...
```mermaid
graph TB
    subgraph shadow_pass1["第一遍: 从光源视角渲染"]
        A1[将摄像机移到光源位置] --> A2[渲染深度图 ShadowMap]
        A2 --> A3[记录每个像素到光源的最近距离]
    end
    
    subgraph shadow_pass2["第二遍: 从玩家视角渲染"]
